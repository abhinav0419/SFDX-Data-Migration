...
#               using SFDX and CSV
# @created:     27/12/2019
# @update :     09/03/2020 | Get child based on parent selected
# @update :     30/03/2020 | Get input from user about type of envr and object file
...
def executeQuery(query):
    return executeCommand("sfdx force:data:soql:query -u {0} -q \"{1}\"".format(srcOrg, query))["records"]
...
#----------------------- IMPORT ------------------#
def importData():
    importRecords = executeCommand("sfdx force:data:tree:import -u {0}  --json -p {1}".format(destOrg, MAIN_PLAN))
...
# ----------------- USER INPUTS -----------
def getObjectsFile():
    objects_file = input("Enter object listing file: ")

    for record in readFile(objects_file):
        mapSobjectDetails[record["name"]] = record

#-------------------------- * MAIN EXECUTION * ---------------------------#

#user inputs
src = input("Enter source org alias: ")
dest = input("Enter destination org alias: ")
...
#Memory Error
del mapIdRef
del mapSobjectLookups

endTime = executionTime("Export Time", startTime)
...
if len(mapSobjectCSVRecords) > 0:
    if CSV_FOLDER not in os.listdir():
        os.mkdir(CSV_FOLDER)

    for sobject in mapSobjectCSVRecords:      
        toCSV(sobject)
